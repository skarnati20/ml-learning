...
class MnistDataloader:
    """
    Modified MNIST data loader taken from Kaggle in order to suit the
    specifications of this repo's neural net implementation.
    """
    ...
    def read_images_labels(self, images_filepath, labels_filepath, num_images):
        ...
        # Only return the specified number
    ...
    def load_data(self, num_images):
        x_train, y_train = self.read_images_labels(
            self.training_images_filepath, self.training_labels_filepath, num_images
        )
        x_test, y_test = self.read_images_labels(
            self.test_images_filepath, self.test_labels_filepath, num_images
        )
        ...
        y_train = np.array(
            [np.matrix(self.convert_to_result_arr(i)).T for i in y_train]
        )
